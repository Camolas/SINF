...
<?php
	...
	if (!empty($_GET["entity"]) && !empty($_GET["serie"]) && !empty($_GET["prod_code"]) && !empty($_GET["prod_quant"]))
	{
		...
		{
			array_push($products, array('CodArtigo' =>$_GET["prod_code"]["$i"],'Quantidade' =>(int)$_GET["prod_quant"]["$i"],'Desconto' => (int)$_GET["prod_discount"]["$i"],'PrecoUnitario' => (int)$_GET["prod_unit_price"]["$i"]));
		}
				...
				<table id="form">
					<tbody>
						<tr class="product">
							...
							<td><input class="form-control" name="prod_unit_price[]" placeholder="Product Unit Price"></td>
						</tr>
					</tbody>
				</table>
				<br>
